...
<html>
    ...
    <body>
        ...
        <section id="blog-home" class="pt-5 mt-0 container">
            <h2 class="font-weight-bold pt-5">Danh sách đơn hàng</h2>
            <hr>
            <c:if test="${requestScope.listo.size()==0}">
                <h2 class="font-weight-bold pt-5" style="text-align: center;color: red">Đơn hàng của bạn còn trống</h2>
                    ...
                    <tbody>
                        <c:forEach items="${requestScope.listo}" var="listo"> 
                            <tr style="cursor: pointer" onclick="window.location = 'showdetail?id=${listo.id}&total=${listo.total}'">
                                ...
                                <td>${listo.address}</td>
                                <td>${listo.total} ₫</td>
                                <td>${fn:substring(listo.orderdate, 0, 10)}</td>
                                <td>${fn:substring(listo.shipdate, 0, 10)}</td>
                                <td>${listo.status==true?"Đã giao":"Đang vận chuyển"}</td>
                            </tr>
                        </c:forEach>
