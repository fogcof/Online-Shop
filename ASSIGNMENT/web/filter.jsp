    ...
    <body>
            ...
            <div class="container">
                ...
                <div class="collapse navbar-collapse" id="navbarSupportedContent">
                    ...
                    <form action="search" class="form-inline my-2 my-lg-0 ml-auto"> <!-- fix -->
                        <input class="form-control mr-sm-2" type="search" name="key" placeholder="Tìm kiếm" aria-label="Search" value="${requestScope.key}">
        ...
        <section id="featured" class="my-5 py-5">
            <div class="container mt-5 py-5 d-flex justify-content-between">
                <div>
                    <h2 class="font-weight-bold">SẢN PHẨM ĐƯỢC LỌC</h2> <!-- fix -->
                    <hr>
                </div>
                ...
                <div class="d-flex justify-content-around">
                    ...
                    <form action="filter">
                        ...
                        <select onchange="this.form.submit()" name="order" class="form-select p-2" aria-label="Default select example">
                            <option ${requestScope.order=="default"?"selected":""} value="default">Thứ tự mặc định</option>
                            <option ${requestScope.order=="asc"?"selected":""} value="asc">Giá từ thấp đến cao</option>
                            <option ${requestScope.order=="desc"?"selected":""} value="desc">Giá từ cao đến thấp</option>
                        </select>
                    </form>
                </div>
            </div>
            <div class="row container mx-auto">
                <div class="wrapper mb-5 mx-auto col-lg-3">
                    <div class="price-input">
                        <div class="field">
                            <!--<span>Min</span>-->
                            <input id="from" type="number" class="input-min" name="from" value="${requestScope.from}">&nbsp;₫
                        </div>
                        <div class="separator">-</div>
                        <div class="field">
                            <!--<span>Max</span>-->
                            <input id="to" type="number" class="input-max" name="to" value="${requestScope.to}">&nbsp;₫
                        </div>
                    </div>
                    <div class="slider">
                        <div class="progress" style="left:${requestScope.from*100/20000000}%;right: ${(20000000-requestScope.to)*100/20000000}%"></div>
                    </div>
                    <div class="range-input d-flex justify-content-around">
                        <input type="range" class="range-min" min="0" max="20000000" value="${requestScope.from}" step="10000">
                        <input type="range" class="range-max" min="0" max="20000000" value="${requestScope.to}" step="10000">
                    ...
                    <nav class="col-12" aria-label="...">
                        <ul class="pagination mt-5">
                            <li class="page-item ${page==1?"disabled":""}">
                                <a class="page-link" href="filter?page=${page-1}&key=${requestScope.key}&order=${requestScope.order}&cid=${requestScope.cid}&from=${from}&to=${to}">Trước</a>
                            </li>
                            <c:forEach begin="1" end="${requestScope.num}" var="num">
                                <li class="page-item <c:if test="${num==page}">active</c:if>">
                                    <a class="page-link" href="filter?page=${num}&key=${requestScope.key}&order=${requestScope.order}&cid=${requestScope.cid}&from=${from}&to=${to}">${num}</a>
                                </li>
                            </c:forEach>
                            <li class="page-item ${page==num?"disabled":""}">
                                <a class="page-link" href="filter?page=${page+1}&key=${requestScope.key}&order=${requestScope.order}&cid=${requestScope.cid}&from=${from}&to=${to}">Sau</a>
                            </li>
                        </ul>
                    </nav>
                </div>
