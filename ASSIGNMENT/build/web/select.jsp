        ...
        <section id="featured" class="my-5 py-5">
            <div class="container mt-5 py-5 d-flex justify-content-between">
                <div>
                    ...
                    <c:if test="${requestScope.key==null}">
                        <c:if test="${cid==0}">
                            <h2 class="font-weight-bold">TẤT CẢ SẢN PHẨM</h2> <!-- fix -->
                            <hr>
                        </c:if>
                        <c:if test="${cid!=0}">
                            <h2 class="font-weight-bold">${requestScope.listp.get(0).category.name}</h2> <!-- fix -->
                            <hr>
                            <p>${requestScope.listp.get(0).category.desc}</p>
                        </c:if>    
                    </c:if>
                </div>
                        ...
                        <ul class="pagination mt-5">
                            <c:if test="${requestScope.key!=null}">
                                <li class="page-item ${page==1?"disabled":""}">
                                    <a class="page-link" href="select?page=${page-1}&key=${requestScope.key}&order=${requestScope.order}">Trước</a>
                                </li>
                                <c:forEach begin="1" end="${requestScope.num}" var="num">
                                    <li class="page-item <c:if test="${num==page}">active</c:if>">
                                        <a class="page-link" href="select?page=${num}&key=${requestScope.key}&order=${requestScope.order}">${num}</a>
                                    </li>
                                </c:forEach>
                                <li class="page-item ${page==num?"disabled":""}">
                                    <a class="page-link" href="select?page=${page+1}&key=${requestScope.key}&order=${requestScope.order}">Sau</a>
                                </li>    
                            </c:if>
                            <c:if test="${requestScope.key==null}">
                                <li class="page-item ${page==1?"disabled":""}">
                                    <a class="page-link" href="select?page=${page-1}&cid=${cid}&order=${requestScope.order}">Trước</a>
                                </li>
                                <c:forEach begin="1" end="${requestScope.num}" var="num">
                                    <li class="page-item <c:if test="${num==page}">active</c:if>">
                                        <a class="page-link" href="select?page=${num}&cid=${cid}&order=${requestScope.order}">${num}</a>
                                    </li>
                                </c:forEach>
                                <li class="page-item ${page==num?"disabled":""}">
                                    <a class="page-link" href="select?page=${page+1}&cid=${cid}&order=${requestScope.order}">Sau</a>
                                </li>    
                            </c:if>
                        </ul>
                    </nav>
